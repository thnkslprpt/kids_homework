// Generated from eHebrew common-words source.
const RAW_HEBREW_WORDS = [
  {
    "category": "Adjectives, Adverbs",
    "english": "A little",
    "transliteration": "K-tsat",
    "hebrew": "קצת",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "A lot",
    "transliteration": "Har-be",
    "hebrew": "הרבה",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Again",
    "transliteration": "Shuv",
    "hebrew": "שוב",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "All",
    "transliteration": "Kol",
    "hebrew": "כל",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Alone",
    "transliteration": "Le-vad",
    "hebrew": "לבד",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Also",
    "transliteration": "Gam",
    "hebrew": "גם",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Angry",
    "transliteration": "Ko-es",
    "hebrew": "כועס",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Bad",
    "transliteration": "Ra",
    "hebrew": "רע",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Beautiful",
    "transliteration": "Ya-fe",
    "hebrew": "יפה",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Big",
    "transliteration": "Ga-dol",
    "hebrew": "גדול",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Black",
    "transliteration": "Sha-khor",
    "hebrew": "שחור",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Blue",
    "transliteration": "Ka-chol",
    "hebrew": "כחול",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Boring",
    "transliteration": "Me-sha-a-mem",
    "hebrew": "משעמם",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Cheap",
    "transliteration": "Zol",
    "hebrew": "זול",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Clean",
    "transliteration": "Na-ki",
    "hebrew": "נקי",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Closed",
    "transliteration": "Sa-gur",
    "hebrew": "סגור",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Cold",
    "transliteration": "Kar",
    "hebrew": "קר",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Correct",
    "transliteration": "Na-chon",
    "hebrew": "נכון",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Difficult",
    "transliteration": "Ka-she",
    "hebrew": "קשה",
    "difficulty": 2
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Down",
    "transliteration": "Le-ma-ta",
    "hebrew": "למטה",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Early",
    "transliteration": "Muk-dam",
    "hebrew": "מוקדם",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Easy",
    "transliteration": "Kal",
    "hebrew": "קל",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Exciting",
    "transliteration": "Mal-hiv",
    "hebrew": "מלהיב",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Expensive",
    "transliteration": "Ya-kar",
    "hebrew": "יקר",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Fast",
    "transliteration": "Ma-hair",
    "hebrew": "מהיר",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Fat",
    "transliteration": "Sha-men",
    "hebrew": "שמן",
    "difficulty": 2
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "First",
    "transliteration": "Ri-shon",
    "hebrew": "ראשון",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Forbidden",
    "transliteration": "A-sur",
    "hebrew": "אסור",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Fried",
    "transliteration": "Me-tu-gan",
    "hebrew": "מטוגן",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Full",
    "transliteration": "Ma-le",
    "hebrew": "מלא",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Funny",
    "transliteration": "Mats-chik",
    "hebrew": "מצחיק",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Good",
    "transliteration": "Tov",
    "hebrew": "טוב",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Green",
    "transliteration": "Ya-rok",
    "hebrew": "ירוק",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Hard",
    "transliteration": "Ka-she",
    "hebrew": "קשה",
    "difficulty": 2
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Half (of something)",
    "transliteration": "Che-tsi",
    "hebrew": "חצי",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Healthy",
    "transliteration": "Ba-ri",
    "hebrew": "בריא",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Here",
    "transliteration": "Po",
    "hebrew": "פה",
    "difficulty": 2
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "High",
    "transliteration": "Ga-vo-ha",
    "hebrew": "גבוה",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Hot",
    "transliteration": "Kham",
    "hebrew": "חם",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Hungry",
    "transliteration": "Ra-ev",
    "hebrew": "רעב",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Important",
    "transliteration": "Cha-shuv",
    "hebrew": "חשוב",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Inside",
    "transliteration": "Bif-nim",
    "hebrew": "בפנים",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Intelligent",
    "transliteration": "Na-von",
    "hebrew": "נבון",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Interesting",
    "transliteration": "Me-an-yen",
    "hebrew": "מעניין",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Jewish",
    "transliteration": "Ye-hu-di",
    "hebrew": "יהודי",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Kosher",
    "transliteration": "Ka-sher",
    "hebrew": "כשר",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Last, final",
    "transliteration": "A-cha-ron",
    "hebrew": "אחרון",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Late",
    "transliteration": "Me-u-char",
    "hebrew": "מאוחר",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Lazy",
    "transliteration": "Ats-lan",
    "hebrew": "עצלן",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Left (direction)",
    "transliteration": "Smo-la",
    "hebrew": "שמאלה",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Low",
    "transliteration": "Na-mukh",
    "hebrew": "נמוך",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Married",
    "transliteration": "Na-su-i",
    "hebrew": "נשוי",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Medium",
    "transliteration": "Bei-no-ni",
    "hebrew": "בינוני",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "New",
    "transliteration": "Cha-dash",
    "hebrew": "חדש",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Next",
    "transliteration": "Ha-ba",
    "hebrew": "הבא",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Noisy",
    "transliteration": "Ro-esh",
    "hebrew": "רועש",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Okay",
    "transliteration": "Be-se-der",
    "hebrew": "בסדר",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Old (person)",
    "transliteration": "Za-ken",
    "hebrew": "זקן",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Old (thing)",
    "transliteration": "Ya-shan",
    "hebrew": "ישן",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Open",
    "transliteration": "Pa-tu-akh",
    "hebrew": "פתוח",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Orange (color)",
    "transliteration": "Ka-tom",
    "hebrew": "כתום",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Outside",
    "transliteration": "Ba-chuts",
    "hebrew": "בחוץ",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Poor",
    "transliteration": "A-ni",
    "hebrew": "עני",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Purple",
    "transliteration": "Sa-gol",
    "hebrew": "סגול",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Quarter (of something)",
    "transliteration": "Re-va",
    "hebrew": "רבע",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Quickly",
    "transliteration": "Ma-hair",
    "hebrew": "מהר",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Quiet",
    "transliteration": "She-ket",
    "hebrew": "שקט",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Ready",
    "transliteration": "Mu-chan",
    "hebrew": "מוכן",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Red",
    "transliteration": "A-dom",
    "hebrew": "אדום",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Religious",
    "transliteration": "Da-ti",
    "hebrew": "דתי",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Rich",
    "transliteration": "A-shir",
    "hebrew": "עשיר",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Right (direction)",
    "transliteration": "Ya-mi-na",
    "hebrew": "ימינה",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Secular",
    "transliteration": "Chi-lo-ni",
    "hebrew": "חילוני",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Sick",
    "transliteration": "Cho-le",
    "hebrew": "חולה",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Simple",
    "transliteration": "Pa-shut",
    "hebrew": "פשוט",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Singular; single (not married)",
    "transliteration": "Ya-chid",
    "hebrew": "יחיד",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Slow",
    "transliteration": "I-ti",
    "hebrew": "איטי",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Slowly",
    "transliteration": "Le-at",
    "hebrew": "לאט",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Small",
    "transliteration": "Ka-tan",
    "hebrew": "קטן",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Smart",
    "transliteration": "Pi-ke-ach",
    "hebrew": "פיקח",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Smelly",
    "transliteration": "Mas-ri-ach",
    "hebrew": "מסריח",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Soft",
    "transliteration": "Rach",
    "hebrew": "רך",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Soon",
    "transliteration": "Be-ka-rov",
    "hebrew": "בקרוב",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Spicy (hot)",
    "transliteration": "Cha-rif",
    "hebrew": "חריף",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Straight ahead; honest",
    "transliteration": "Ya-shar",
    "hebrew": "ישר",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Strong",
    "transliteration": "Cha-zak",
    "hebrew": "חזק",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Sweet",
    "transliteration": "Ma-tok",
    "hebrew": "מתוק",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Tasty",
    "transliteration": "Ta-im",
    "hebrew": "טעים",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "There",
    "transliteration": "Sham",
    "hebrew": "שם",
    "difficulty": 2
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Thin",
    "transliteration": "Ra-ze",
    "hebrew": "רזה",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Third (of something)",
    "transliteration": "Shlish",
    "hebrew": "שליש",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Tired",
    "transliteration": "A-yef",
    "hebrew": "עייף",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Today",
    "transliteration": "Ha-yom",
    "hebrew": "היום",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Together",
    "transliteration": "Be-ya-chad",
    "hebrew": "ביחד",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Tomorrow",
    "transliteration": "Ma-char",
    "hebrew": "מחר",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Twice",
    "transliteration": "Pa-a-ma-yim",
    "hebrew": "פעמיים",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Ugly",
    "transliteration": "Me-cho-ar",
    "hebrew": "מכוער",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Up",
    "transliteration": "Le-ma-la",
    "hebrew": "למעלה",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Weak",
    "transliteration": "Cha-lash",
    "hebrew": "חלש",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Very",
    "transliteration": "Me-od",
    "hebrew": "מאוד",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Wet",
    "transliteration": "Ra-tuv",
    "hebrew": "רטוב",
    "difficulty": 1
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "White",
    "transliteration": "La-van",
    "hebrew": "לבן",
    "difficulty": 2
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Who",
    "transliteration": "Mi",
    "hebrew": "מי",
    "difficulty": 2
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Whole",
    "transliteration": "Sha-laim",
    "hebrew": "שלם",
    "difficulty": 2
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Without",
    "transliteration": "Bli",
    "hebrew": "בלי",
    "difficulty": 2
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Yellow",
    "transliteration": "Tsa-hov",
    "hebrew": "צהוב",
    "difficulty": 2
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Yes",
    "transliteration": "Ken",
    "hebrew": "כן",
    "difficulty": 2
  },
  {
    "category": "Adjectives, Adverbs",
    "english": "Yesterday",
    "transliteration": "Et-mol",
    "hebrew": "אתמול",
    "difficulty": 2
  },
  {
    "category": "Conjunctions, prepositions",
    "english": "And",
    "transliteration": "Ve",
    "hebrew": "ו-",
    "difficulty": 2
  },
  {
    "category": "Conjunctions, prepositions",
    "english": "Before",
    "transliteration": "Lif-nei",
    "hebrew": "לפני",
    "difficulty": 2
  },
  {
    "category": "Conjunctions, prepositions",
    "english": "But",
    "transliteration": "A-val",
    "hebrew": "אבל",
    "difficulty": 2
  },
  {
    "category": "Conjunctions, prepositions",
    "english": "By, with, at the place of",
    "transliteration": "Ets-el",
    "hebrew": "אצל",
    "difficulty": 2
  },
  {
    "category": "Conjunctions, prepositions",
    "english": "From",
    "transliteration": "Me",
    "hebrew": "מ-",
    "difficulty": 2
  },
  {
    "category": "Conjunctions, prepositions",
    "english": "If",
    "transliteration": "Im",
    "hebrew": "אם",
    "difficulty": 2
  },
  {
    "category": "Conjunctions, prepositions",
    "english": "In, at",
    "transliteration": "Be, ba",
    "hebrew": "ב-",
    "difficulty": 2
  },
  {
    "category": "Conjunctions, prepositions",
    "english": "Like/ as",
    "transliteration": "Ke",
    "hebrew": "כ-",
    "difficulty": 3
  },
  {
    "category": "Conjunctions, prepositions",
    "english": "Maybe",
    "transliteration": "U-lai",
    "hebrew": "אולי",
    "difficulty": 2
  },
  {
    "category": "Conjunctions, prepositions",
    "english": "No",
    "transliteration": "Lo",
    "hebrew": "לא",
    "difficulty": 2
  },
  {
    "category": "Conjunctions, prepositions",
    "english": "Now",
    "transliteration": "Ach-shav",
    "hebrew": "עכשיו",
    "difficulty": 2
  },
  {
    "category": "Conjunctions, prepositions",
    "english": "Of",
    "transliteration": "Shel",
    "hebrew": "של",
    "difficulty": 2
  },
  {
    "category": "Conjunctions, prepositions",
    "english": "On, about",
    "transliteration": "Al",
    "hebrew": "על",
    "difficulty": 2
  },
  {
    "category": "Conjunctions, prepositions",
    "english": "Once",
    "transliteration": "Pa-am",
    "hebrew": "פעם",
    "difficulty": 2
  },
  {
    "category": "Conjunctions, prepositions",
    "english": "Only",
    "transliteration": "Rak",
    "hebrew": "רק",
    "difficulty": 2
  },
  {
    "category": "Conjunctions, prepositions",
    "english": "Or",
    "transliteration": "O",
    "hebrew": "או",
    "difficulty": 2
  },
  {
    "category": "Conjunctions, prepositions",
    "english": "So",
    "transliteration": "Az",
    "hebrew": "אז",
    "difficulty": 2
  },
  {
    "category": "Conjunctions, prepositions",
    "english": "The",
    "transliteration": "Ha",
    "hebrew": "ה-",
    "difficulty": 2
  },
  {
    "category": "Conjunctions, prepositions",
    "english": "This",
    "transliteration": "Ze",
    "hebrew": "זה",
    "difficulty": 2
  },
  {
    "category": "Conjunctions, prepositions",
    "english": "To, for",
    "transliteration": "Le",
    "hebrew": "ל-",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Address",
    "transliteration": "Ktov-et",
    "hebrew": "כתובת",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Air conditioner",
    "transliteration": "Maz-gan",
    "hebrew": "מזגן",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Airplane",
    "transliteration": "Ma-tos",
    "hebrew": "מטוס",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Airport",
    "transliteration": "Na-mal te-u-fa",
    "hebrew": "נמל תעופה",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Almond",
    "transliteration": "Sha-ked",
    "hebrew": "שקד",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Answer",
    "transliteration": "Tshu-va",
    "hebrew": "תשובה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Ant",
    "transliteration": "Ne-ma-la",
    "hebrew": "נמלה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Apartment",
    "transliteration": "Di-ra",
    "hebrew": "דירה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Apple",
    "transliteration": "Ta-pu-akh",
    "hebrew": "תפוח",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Arabic",
    "transliteration": "A-ra-vit",
    "hebrew": "ערבית",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Arm",
    "transliteration": "Yad",
    "hebrew": "יד",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "ATM",
    "transliteration": "Kas-po-mat",
    "hebrew": "כספומט",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Aunt",
    "transliteration": "Do-da",
    "hebrew": "דודה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Baby",
    "transliteration": "Ti-nok",
    "hebrew": "תינוק",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Back",
    "transliteration": "Gav",
    "hebrew": "גב",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Bag",
    "transliteration": "Tik",
    "hebrew": "תיק",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Bakery",
    "transliteration": "Ma-a-fi-ya",
    "hebrew": "מאפייה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Ball",
    "transliteration": "Ka-dur",
    "hebrew": "כדור",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Banana",
    "transliteration": "Ba-na-na",
    "hebrew": "בננה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Bank",
    "transliteration": "Bank",
    "hebrew": "בנק",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Basketball",
    "transliteration": "Ka-dur-sal",
    "hebrew": "כדורסל",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Beach",
    "transliteration": "Khof",
    "hebrew": "חוף",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Bed",
    "transliteration": "Mi-ta",
    "hebrew": "מיטה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Beef",
    "transliteration": "Ba-kar",
    "hebrew": "בקר",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Beer",
    "transliteration": "Bi-ra",
    "hebrew": "בירה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Beetroot",
    "transliteration": "Se-lek",
    "hebrew": "סלק",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Belt",
    "transliteration": "Cha-go-ra",
    "hebrew": "חגורה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Bicycle",
    "transliteration": "O-fa-na-yim",
    "hebrew": "אופניים",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Bill, check",
    "transliteration": "Khesh-bon",
    "hebrew": "חשבון",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Bird",
    "transliteration": "Tsi-por",
    "hebrew": "ציפור",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Body",
    "transliteration": "Guf",
    "hebrew": "גוף",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Bone",
    "transliteration": "E-tsem",
    "hebrew": "עצם",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Book",
    "transliteration": "Se-fer",
    "hebrew": "ספר",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Bottle",
    "transliteration": "Bak-buk",
    "hebrew": "בקבוק",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Bowl",
    "transliteration": "Ke-a-ra",
    "hebrew": "קערה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Boy",
    "transliteration": "Ye-led",
    "hebrew": "ילד",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Brain",
    "transliteration": "Mo-akh",
    "hebrew": "מוח",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Bread",
    "transliteration": "Le-khem",
    "hebrew": "לחם",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Break",
    "transliteration": "Haf-sa-ka",
    "hebrew": "הפסקה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Breakfast",
    "transliteration": "A-ru-chat bo-ker",
    "hebrew": "ארוחת בוקר",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Brother",
    "transliteration": "Ach",
    "hebrew": "אח",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Brown",
    "transliteration": "Khum",
    "hebrew": "חום",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Building",
    "transliteration": "Bin-yan",
    "hebrew": "בניין",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Bus",
    "transliteration": "O-to-bus",
    "hebrew": "אוטובוס",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Butter",
    "transliteration": "Chem-a",
    "hebrew": "חמאה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Cabbage",
    "transliteration": "Kruv",
    "hebrew": "כרוב",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Cake",
    "transliteration": "U-ga",
    "hebrew": "עוגה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Camera",
    "transliteration": "Mats-le-ma",
    "hebrew": "מצלמה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Car",
    "transliteration": "Me-kho-nit",
    "hebrew": "מכונית",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Carrot",
    "transliteration": "Ge-zer",
    "hebrew": "גזר",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Cat",
    "transliteration": "Cha-tul",
    "hebrew": "חתול",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Ceiling",
    "transliteration": "Tik-ra",
    "hebrew": "תקרה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Chair",
    "transliteration": "Ki-se",
    "hebrew": "כיסא",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Challah",
    "transliteration": "Cha-la",
    "hebrew": "חלה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Channel (tv)",
    "transliteration": "A-ruts",
    "hebrew": "ערוץ",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Chapter",
    "transliteration": "Pe-rek",
    "hebrew": "פרק",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Cheese",
    "transliteration": "Gvi-na",
    "hebrew": "גבינה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Cherry",
    "transliteration": "Duv-de-van",
    "hebrew": "דובדבן",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Chest",
    "transliteration": "Cha-ze",
    "hebrew": "חזה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Chicken",
    "transliteration": "Of",
    "hebrew": "עוף",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Chocolate",
    "transliteration": "Sho-ko-lad",
    "hebrew": "שוקולד",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "City",
    "transliteration": "Ir",
    "hebrew": "עיר",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Classroom",
    "transliteration": "Ki-ta",
    "hebrew": "כיתה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Clock",
    "transliteration": "Sha-on",
    "hebrew": "שעון",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Clothes",
    "transliteration": "Be-ga-dim",
    "hebrew": "בגדים",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Cloud",
    "transliteration": "A-nan",
    "hebrew": "ענן",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Cockroach",
    "transliteration": "Juk",
    "hebrew": "ג׳וק",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Coffee",
    "transliteration": "Ka-fe",
    "hebrew": "קפה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Color",
    "transliteration": "Tse-va",
    "hebrew": "צבע",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Company",
    "transliteration": "Chev-ra",
    "hebrew": "חברה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Computer",
    "transliteration": "Mach-shev",
    "hebrew": "מחשב",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Country",
    "transliteration": "Me-di-na",
    "hebrew": "מדינה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Cousin",
    "transliteration": "Ben dod",
    "hebrew": "בן דוד",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Cucumber",
    "transliteration": "Me-la-fe-fon",
    "hebrew": "מלפפון",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Dad",
    "transliteration": "A-ba",
    "hebrew": "אבא",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Date (the fruit)",
    "transliteration": "Ta-mar",
    "hebrew": "תמר",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Daughter",
    "transliteration": "Bat",
    "hebrew": "בת",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Day",
    "transliteration": "Yom",
    "hebrew": "יום",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Desert",
    "transliteration": "Mid-bar",
    "hebrew": "מדבר",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Dictionary",
    "transliteration": "Mi-lon",
    "hebrew": "מילון",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Doctor / physician",
    "transliteration": "Ro-fe",
    "hebrew": "רופא",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Dog",
    "transliteration": "Ke-lev",
    "hebrew": "כלב",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Door",
    "transliteration": "De-let",
    "hebrew": "דלת",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Drink",
    "transliteration": "Shti-ya",
    "hebrew": "שתייה",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Driver",
    "transliteration": "Na-hag",
    "hebrew": "נהג",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Ear",
    "transliteration": "O-zen",
    "hebrew": "אוזן",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "East",
    "transliteration": "Miz-rakh",
    "hebrew": "מזרח",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Egg",
    "transliteration": "Bei-tsa",
    "hebrew": "ביצה",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "End",
    "transliteration": "Sof",
    "hebrew": "סוף",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Engineer",
    "transliteration": "Me-han-des",
    "hebrew": "מהנדס",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "English",
    "transliteration": "An-glit",
    "hebrew": "אנגלית",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Entrance",
    "transliteration": "Kni-sa",
    "hebrew": "כניסה",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Evening",
    "transliteration": "E-rev",
    "hebrew": "ערב",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Excellent",
    "transliteration": "Me-tsu-yan",
    "hebrew": "מצוין",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Exercise",
    "transliteration": "Hit-am-lut",
    "hebrew": "התעמלות",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Exit",
    "transliteration": "Ye-tsi-a",
    "hebrew": "יציאה",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Eye",
    "transliteration": "A-yin",
    "hebrew": "עין",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Face",
    "transliteration": "Pa-nim",
    "hebrew": "פנים",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Faculty",
    "transliteration": "Fa-kul-ta",
    "hebrew": "פקולטה",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Falafel",
    "transliteration": "Fa-la-fel",
    "hebrew": "פלאפל",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Fall (the season)",
    "transliteration": "Stav",
    "hebrew": "סתיו",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Family",
    "transliteration": "Mish-pa-cha",
    "hebrew": "משפחה",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Fig",
    "transliteration": "Te-e-na",
    "hebrew": "תאנה",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Toe",
    "transliteration": "Ets-ba",
    "hebrew": "אצבע",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Finger",
    "transliteration": "Ets-ba",
    "hebrew": "אצבע",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Fish",
    "transliteration": "Dag",
    "hebrew": "דג",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Floor",
    "transliteration": "Rits-pa",
    "hebrew": "רצפה",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Food",
    "transliteration": "O-chel",
    "hebrew": "אוכל",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Foot",
    "transliteration": "Kaf re-gel",
    "hebrew": "כף רגל",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Fork",
    "transliteration": "Maz-leg",
    "hebrew": "מזלג",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Friday",
    "transliteration": "Yom shi-shi",
    "hebrew": "יום שישי",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Friend",
    "transliteration": "Cha-ver",
    "hebrew": "חבר",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Fruit",
    "transliteration": "Pri",
    "hebrew": "פרי",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Future",
    "transliteration": "A-tid",
    "hebrew": "עתיד",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Game",
    "transliteration": "Mis-chak",
    "hebrew": "משחק",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Garbage can",
    "transliteration": "Pach ash-pa",
    "hebrew": "פח אשפה",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Garlic",
    "transliteration": "Shoom",
    "hebrew": "שום",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Gasoline",
    "transliteration": "De-lek",
    "hebrew": "דלק",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Gate",
    "transliteration": "Sha-ar",
    "hebrew": "שער",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Girl",
    "transliteration": "Yal-da",
    "hebrew": "ילדה",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Glass, cup",
    "transliteration": "Kos",
    "hebrew": "כוס",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Government",
    "transliteration": "Mem-sha-la",
    "hebrew": "ממשלה",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Grandfather",
    "transliteration": "Sa-ba",
    "hebrew": "סבא",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Grandmother",
    "transliteration": "Sav-ta",
    "hebrew": "סבתא",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Grapefruit",
    "transliteration": "Esh-ko-lit",
    "hebrew": "אשכולית",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Grapes",
    "transliteration": "A-na-vim",
    "hebrew": "ענבים",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Group",
    "transliteration": "Kvu-tsa",
    "hebrew": "קבוצה",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Hair",
    "transliteration": "Se-ar",
    "hebrew": "שער",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Hand",
    "transliteration": "Yad",
    "hebrew": "יד",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Hat",
    "transliteration": "Ko-va",
    "hebrew": "כובע",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "He",
    "transliteration": "Hu",
    "hebrew": "הוא",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Head",
    "transliteration": "Rosh",
    "hebrew": "ראש",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Heart",
    "transliteration": "Lev",
    "hebrew": "לב",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Hebrew",
    "transliteration": "Iv-rit",
    "hebrew": "עברית",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Height",
    "transliteration": "Go-va",
    "hebrew": "גובה",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Hobby",
    "transliteration": "Takh-biv",
    "hebrew": "תחביב",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Holiday",
    "transliteration": "Khag",
    "hebrew": "חג",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Honey",
    "transliteration": "Dvash",
    "hebrew": "דבש",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Hospital",
    "transliteration": "Beit kho-lim",
    "hebrew": "בית חולים",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Hot chocolate",
    "transliteration": "Sho-ko kham",
    "hebrew": "שוקו חם",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Hotel",
    "transliteration": "Ma-lon",
    "hebrew": "מלון",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Hour",
    "transliteration": "Sha-a",
    "hebrew": "שעה",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "House",
    "transliteration": "Ba-yit",
    "hebrew": "בית",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Hummus",
    "transliteration": "Chu-mus",
    "hebrew": "חומוס",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Husband",
    "transliteration": "Ba-al",
    "hebrew": "בעל",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "I",
    "transliteration": "A-ni",
    "hebrew": "אני",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "IDF (Israel Defense Forces)",
    "transliteration": "Tsa-hal",
    "hebrew": "צה״ל",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Ice cream",
    "transliteration": "Gli-da",
    "hebrew": "גלידה",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Idea",
    "transliteration": "Ra-a-yon",
    "hebrew": "רעיון",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Israel",
    "transliteration": "Is-ra-el",
    "hebrew": "ישראל",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Juice",
    "transliteration": "Mits",
    "hebrew": "מיץ",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Key",
    "transliteration": "Maf-te-ach",
    "hebrew": "מפתח",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Kitchen",
    "transliteration": "Mit-bakh",
    "hebrew": "מטבח",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Knife",
    "transliteration": "Sa-kin",
    "hebrew": "סכין",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Lamb",
    "transliteration": "Ta-le",
    "hebrew": "טלה",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Language",
    "transliteration": "Sa-fa",
    "hebrew": "שפה",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Leg",
    "transliteration": "Re-gel",
    "hebrew": "רגל",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Lemon",
    "transliteration": "Li-mon",
    "hebrew": "לימון",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Lesson",
    "transliteration": "Shi-ur",
    "hebrew": "שיעור",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Lettuce",
    "transliteration": "Cha-sa",
    "hebrew": "חסה",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Library",
    "transliteration": "Sif-ri-ya",
    "hebrew": "ספרייה",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Lizard",
    "transliteration": "Le-ta-a",
    "hebrew": "לטאה",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Mail; post office",
    "transliteration": "Do-ar",
    "hebrew": "דואר",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Mall",
    "transliteration": "Kan-yon",
    "hebrew": "קניון",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Manager",
    "transliteration": "Me-na-hel",
    "hebrew": "מנהל",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Mango",
    "transliteration": "Man-go",
    "hebrew": "מנגו",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Meat",
    "transliteration": "Ba-sar",
    "hebrew": "בשר",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Meter",
    "transliteration": "Me-ter",
    "hebrew": "מטר",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Microwave",
    "transliteration": "Mik-ro-gal",
    "hebrew": "מיקרוגל",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Milk",
    "transliteration": "Cha-lav",
    "hebrew": "חלב",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Minute",
    "transliteration": "Da-ka",
    "hebrew": "דקה",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Mold",
    "transliteration": "O-vesh",
    "hebrew": "עובש",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Mom",
    "transliteration": "I-ma",
    "hebrew": "אימא",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Moment",
    "transliteration": "Re-ga",
    "hebrew": "רגע",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Monday",
    "transliteration": "Yom she-ni",
    "hebrew": "יום שני",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Money",
    "transliteration": "Ke-sef",
    "hebrew": "כסף",
    "difficulty": 3
  },
  {
    "category": "Nouns",
    "english": "Month",
    "transliteration": "Cho-desh",
    "hebrew": "חודש",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Moon",
    "transliteration": "Ya-re-akh",
    "hebrew": "ירח",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Morning",
    "transliteration": "Bo-ker",
    "hebrew": "בוקר",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Mosquito",
    "transliteration": "Ya-tush",
    "hebrew": "יתוש",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Mouth",
    "transliteration": "Pe",
    "hebrew": "פה",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Movie",
    "transliteration": "Se-ret",
    "hebrew": "סרט",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Museum",
    "transliteration": "Mu-zei-on",
    "hebrew": "מוזיאון",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Mushrooms",
    "transliteration": "Pit-ri-yot",
    "hebrew": "פטריות",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Music",
    "transliteration": "Mu-zi-ka",
    "hebrew": "מוזיקה",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Name",
    "transliteration": "Shem",
    "hebrew": "שם",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Napkin",
    "transliteration": "Ma-pit",
    "hebrew": "מפית",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Nectarine",
    "transliteration": "Nek-ta-ri-na",
    "hebrew": "נקטרינה",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Neighborhood",
    "transliteration": "Sh’chu-na",
    "hebrew": "שכונה",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "News",
    "transliteration": "Cha-da-shot",
    "hebrew": "חדשות",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Newspaper",
    "transliteration": "I-ton",
    "hebrew": "עיתון",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Night",
    "transliteration": "Lai-la",
    "hebrew": "לילה",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Noon",
    "transliteration": "Tso-ho-ra-yim",
    "hebrew": "צהריים",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "North",
    "transliteration": "Tsa-fon",
    "hebrew": "צפון",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Nose",
    "transliteration": "Af",
    "hebrew": "אף",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Notebook",
    "transliteration": "Mach-be-ret",
    "hebrew": "מחברת",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Nothing",
    "transliteration": "Klum",
    "hebrew": "כלום",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Number",
    "transliteration": "Mis-par",
    "hebrew": "מספר",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Office",
    "transliteration": "Mis-rad",
    "hebrew": "משרד",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Oil",
    "transliteration": "She-men",
    "hebrew": "שמן",
    "difficulty": 2
  },
  {
    "category": "Nouns",
    "english": "Olive",
    "transliteration": "Za-yit",
    "hebrew": "זית",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Onion",
    "transliteration": "Ba-tsal",
    "hebrew": "בצל",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Orange (food)",
    "transliteration": "Ta-puz",
    "hebrew": "תפוז",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Oven",
    "transliteration": "Ta-nur",
    "hebrew": "תנור",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Pain/ache",
    "transliteration": "Ke-ev",
    "hebrew": "כאב",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Palm",
    "transliteration": "Kaf yad",
    "hebrew": "כף יד",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Pants",
    "transliteration": "Mich-na-sa-yim",
    "hebrew": "מכנסיים",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Paper",
    "transliteration": "Ni-yar",
    "hebrew": "נייר",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Parents",
    "transliteration": "Ho-rim",
    "hebrew": "הורים",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Parsley",
    "transliteration": "Pet-ro-zil-ya",
    "hebrew": "פטרוזיליה",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Passport",
    "transliteration": "Dar-kon",
    "hebrew": "דרכון",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Past",
    "transliteration": "A-var",
    "hebrew": "עבר",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Pasta",
    "transliteration": "Pas-ta",
    "hebrew": "פסטה",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Pastry",
    "transliteration": "Ma-a-fe",
    "hebrew": "מאפה",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Peach",
    "transliteration": "A-far-sek",
    "hebrew": "אפרסק",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Pear",
    "transliteration": "A-gas",
    "hebrew": "אגס",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Pen",
    "transliteration": "Et",
    "hebrew": "עט",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Pencil",
    "transliteration": "I-pa-ron",
    "hebrew": "עיפרון",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Pepper",
    "transliteration": "Pil-pel",
    "hebrew": "פלפל",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Pharmacy",
    "transliteration": "Beit mir-ka-chat",
    "hebrew": "בית מרקחת",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Piano",
    "transliteration": "Psan-ter",
    "hebrew": "פסנתר",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Pickles",
    "transliteration": "Cha-mu-tsim",
    "hebrew": "חמוצים",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Picture",
    "transliteration": "Tmu-na",
    "hebrew": "תמונה",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Pillow",
    "transliteration": "Ka-rit",
    "hebrew": "כרית",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Pineapple",
    "transliteration": "A-na-nas",
    "hebrew": "אננס",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Pita",
    "transliteration": "Pi-ta",
    "hebrew": "פיתה",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Place",
    "transliteration": "Ma-kom",
    "hebrew": "מקום",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Plate",
    "transliteration": "Tsa-la-chat",
    "hebrew": "צלחת",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Plum",
    "transliteration": "She-zif",
    "hebrew": "שזיף",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Police",
    "transliteration": "Mish-ta-ra",
    "hebrew": "משטרה",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Pomegranate",
    "transliteration": "Ri-mon",
    "hebrew": "רימון",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Potato",
    "transliteration": "Ta-pu-ach a-da-ma",
    "hebrew": "תפוח אדמה",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Present (tense)",
    "transliteration": "Ho-ve",
    "hebrew": "הווה",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Printer",
    "transliteration": "Mad-pe-set",
    "hebrew": "מדפסת",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Question",
    "transliteration": "She-e-la",
    "hebrew": "שאלה",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Quinoa",
    "transliteration": "Ki-no-a",
    "hebrew": "קינואה",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Rabbi",
    "transliteration": "Rav",
    "hebrew": "רב",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Rain",
    "transliteration": "Ge-shem",
    "hebrew": "גשם",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Receipt",
    "transliteration": "Ka-ba-la",
    "hebrew": "קבלה",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Refrigerator",
    "transliteration": "Me-ka-rer",
    "hebrew": "מקרר",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Restaurant",
    "transliteration": "Mis-a-da",
    "hebrew": "מסעדה",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Rice",
    "transliteration": "O-rez",
    "hebrew": "אורז",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "River",
    "transliteration": "Na-har",
    "hebrew": "נהר",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Road, route",
    "transliteration": "Kvish",
    "hebrew": "כביש",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Room",
    "transliteration": "Che-der",
    "hebrew": "חדר",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Rug",
    "transliteration": "Sha-ti-akh",
    "hebrew": "שטיח",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Salad",
    "transliteration": "Sa-lat",
    "hebrew": "סלט",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Salt",
    "transliteration": "Me-lakh",
    "hebrew": "מלח",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Sandals",
    "transliteration": "San-da-lim",
    "hebrew": "סנדלים",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Saturday",
    "transliteration": "Sha-bat",
    "hebrew": "שבת",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Sauce",
    "transliteration": "Ro-tev",
    "hebrew": "רוטב",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Schnitzel",
    "transliteration": "Shni-tsel",
    "hebrew": "שניצל",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "School",
    "transliteration": "Beit se-fer",
    "hebrew": "בית ספר",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Scientist",
    "transliteration": "Mad-an",
    "hebrew": "מדען",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Sea",
    "transliteration": "Yam",
    "hebrew": "ים",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Serving/portion",
    "transliteration": "Ma-na",
    "hebrew": "מנה",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Shakshuka",
    "transliteration": "Shak-shu-ka",
    "hebrew": "שקשוקה",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Shawarma",
    "transliteration": "Sha-war-ma",
    "hebrew": "שווארמה",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "She",
    "transliteration": "Hi",
    "hebrew": "היא",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Sheet",
    "transliteration": "Sa-din",
    "hebrew": "סדין",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Shekel",
    "transliteration": "She-kel",
    "hebrew": "שקל",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Shirt",
    "transliteration": "Chul-tsa",
    "hebrew": "חולצה",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Shoes",
    "transliteration": "Na-a-la-yim",
    "hebrew": "נעליים",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Shorts",
    "transliteration": "Mich-na-sa-yim k-tsar-im",
    "hebrew": "מכנסיים קצרים",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Shuttle taxi",
    "transliteration": "Shei-rut",
    "hebrew": "שירות",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Sidewalk",
    "transliteration": "Mid-ra-cha",
    "hebrew": "מדרכה",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Sister",
    "transliteration": "A-chot",
    "hebrew": "אחות",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Skirt",
    "transliteration": "Cha-tsa-it",
    "hebrew": "חצאית",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Sky",
    "transliteration": "Sha-ma-yim",
    "hebrew": "שמיים",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Soap",
    "transliteration": "Sa-bon",
    "hebrew": "סבון",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Soccer",
    "transliteration": "Ka-du-re-gel",
    "hebrew": "כדורגל",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Socks",
    "transliteration": "Gar-ba-yim",
    "hebrew": "גרביים",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Sofa",
    "transliteration": "Sa-pa",
    "hebrew": "ספה",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Someone",
    "transliteration": "Mi-she-hu",
    "hebrew": "מישהו",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Something",
    "transliteration": "Ma-she-hu",
    "hebrew": "משהו",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Son",
    "transliteration": "Ben",
    "hebrew": "בן",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Soup",
    "transliteration": "Ma-rak",
    "hebrew": "מרק",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "South",
    "transliteration": "Da-rom",
    "hebrew": "דרום",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Spider",
    "transliteration": "A-ka-vish",
    "hebrew": "עכביש",
    "difficulty": 4
  },
  {
    "category": "Nouns",
    "english": "Spinach",
    "transliteration": "Te-red",
    "hebrew": "תרד",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Spoon",
    "transliteration": "Kaf",
    "hebrew": "כף",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Sport",
    "transliteration": "Sport",
    "hebrew": "ספורט",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Spring",
    "transliteration": "A-viv",
    "hebrew": "אביב",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Squash, zucchini",
    "transliteration": "Ki-shu",
    "hebrew": "קישוא",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Star",
    "transliteration": "Ko-chav",
    "hebrew": "כוכב",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Station, stop",
    "transliteration": "Ta-cha-na",
    "hebrew": "תחנה",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Stomach",
    "transliteration": "Be-ten",
    "hebrew": "בטן",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Stop",
    "transliteration": "A-tsor",
    "hebrew": "עצור",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Store",
    "transliteration": "Kha-nut",
    "hebrew": "חנות",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Story",
    "transliteration": "Si-pur",
    "hebrew": "סיפור",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Strawberry",
    "transliteration": "Tut",
    "hebrew": "תות",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Street",
    "transliteration": "Re-chov",
    "hebrew": "רחוב",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Student",
    "transliteration": "Stu-dent",
    "hebrew": "סטודנט",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Sugar",
    "transliteration": "Su-kar",
    "hebrew": "סוכר",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Summer",
    "transliteration": "Ka-yits",
    "hebrew": "קיץ",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Sun",
    "transliteration": "She-mesh",
    "hebrew": "שמש",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Sunday",
    "transliteration": "Yom ri-shon",
    "hebrew": "יום ראשון",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Supermarket",
    "transliteration": "Su-per-mar-ket",
    "hebrew": "סופרמרקט",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Surname",
    "transliteration": "Shem mish-pa-cha",
    "hebrew": "שם משפחה",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Sweet potato",
    "transliteration": "Ba-ta-ta",
    "hebrew": "בטטה",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Swimming",
    "transliteration": "Schi-ya",
    "hebrew": "שחייה",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Synagogue",
    "transliteration": "Beit kne-set",
    "hebrew": "בית כנסת",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Table",
    "transliteration": "Shul-chan",
    "hebrew": "שולחן",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Taxi",
    "transliteration": "Mo-nit",
    "hebrew": "מונית",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Tea",
    "transliteration": "Te",
    "hebrew": "תה",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Teacher",
    "transliteration": "Mo-re, mo-ra",
    "hebrew": "מורה",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Teeth",
    "transliteration": "Shi-na-yim",
    "hebrew": "שיניים",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Telephone",
    "transliteration": "Te-le-fon",
    "hebrew": "טלפון",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Test/exam",
    "transliteration": "Miv-khan",
    "hebrew": "מבחן",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "They (f.)",
    "transliteration": "Hen",
    "hebrew": "הן",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "They (m.)",
    "transliteration": "Hem",
    "hebrew": "הם",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Thumb",
    "transliteration": "A-gu-dal",
    "hebrew": "אגודל",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Thursday",
    "transliteration": "Yom cha-mi-shi",
    "hebrew": "יום חמישי",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Time",
    "transliteration": "Zman",
    "hebrew": "זמן",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Bathroom / toilets",
    "transliteration": "She-ru-tim",
    "hebrew": "שירותים",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Tomato",
    "transliteration": "Ag-va-ni-ya",
    "hebrew": "עגבנייה",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Towel",
    "transliteration": "Ma-ge-vet",
    "hebrew": "מגבת",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Traffic",
    "transliteration": "Tnu-a",
    "hebrew": "תנועה",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Train",
    "transliteration": "Ra-ke-vet",
    "hebrew": "רכבת",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Tree, wood",
    "transliteration": "Ets",
    "hebrew": "עץ",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Trip, hike",
    "transliteration": "Ti-yul",
    "hebrew": "טיול",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Tuesday",
    "transliteration": "Yom shli-shi",
    "hebrew": "יום שלישי",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Tunnel",
    "transliteration": "Min-ha-ra",
    "hebrew": "מנהרה",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "India",
    "transliteration": "Ho-du",
    "hebrew": "הודו",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Tv",
    "transliteration": "Te-le-viz-ya",
    "hebrew": "טלוויזיה",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Uncle",
    "transliteration": "Dod",
    "hebrew": "דוד",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "University",
    "transliteration": "U-ni-ver-si-ta",
    "hebrew": "אוניברסיטה",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Vacation",
    "transliteration": "Chuf-sha",
    "hebrew": "חופשה",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Vegetables",
    "transliteration": "Ye-ra-kot",
    "hebrew": "ירקות",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Wall",
    "transliteration": "Kir",
    "hebrew": "קיר",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Wallet",
    "transliteration": "Ar-nak",
    "hebrew": "ארנק",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Walnut",
    "transliteration": "E-goz me-lech",
    "hebrew": "אגוז מלך",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Water",
    "transliteration": "Ma-yim",
    "hebrew": "מים",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Watermelon",
    "transliteration": "A-va-ti-ach",
    "hebrew": "אבטיח",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "We",
    "transliteration": "A-nach-nu",
    "hebrew": "אנחנו",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Weapon",
    "transliteration": "Ne-shek",
    "hebrew": "נשק",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Weather",
    "transliteration": "Me-zeg a-vir",
    "hebrew": "מזג אוויר",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Wednesday",
    "transliteration": "Yom re-vi-i",
    "hebrew": "יום רביעי",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Week",
    "transliteration": "Sha-vu-a",
    "hebrew": "שבוע",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Weekend",
    "transliteration": "Sof sha-vu-a",
    "hebrew": "סוף שבוע",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Weight",
    "transliteration": "Mish-kal",
    "hebrew": "משקל",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "West",
    "transliteration": "Ma-a-rav",
    "hebrew": "מערב",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Wife",
    "transliteration": "I-sha",
    "hebrew": "אישה",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Window",
    "transliteration": "Cha-lon",
    "hebrew": "חלון",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Wine",
    "transliteration": "Yay-in",
    "hebrew": "יין",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Winter",
    "transliteration": "Cho-ref",
    "hebrew": "חורף",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "World",
    "transliteration": "O-lam",
    "hebrew": "עולם",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Year",
    "transliteration": "Sha-na",
    "hebrew": "שנה",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "Yogurt",
    "transliteration": "Yo-gurt",
    "hebrew": "יוגורט",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "You (f.)",
    "transliteration": "At",
    "hebrew": "את",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "You (f.p.)",
    "transliteration": "A-ten",
    "hebrew": "אתן",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "You (m.)",
    "transliteration": "A-ta",
    "hebrew": "אתה",
    "difficulty": 5
  },
  {
    "category": "Nouns",
    "english": "You (m.p.)",
    "transliteration": "A-tem",
    "hebrew": "אתם",
    "difficulty": 5
  },
  {
    "category": "Phrases",
    "english": "Bless you",
    "transliteration": "Liv-ri-ut",
    "hebrew": "לבריאות",
    "difficulty": 5
  },
  {
    "category": "Phrases",
    "english": "Cheers!",
    "transliteration": "Le-cha-yim",
    "hebrew": "לחיים",
    "difficulty": 5
  },
  {
    "category": "Phrases",
    "english": "Date of birth",
    "transliteration": "Ta-a-rich lei-da",
    "hebrew": "תאריך לידה",
    "difficulty": 5
  },
  {
    "category": "Phrases",
    "english": "Dinner",
    "transliteration": "A-ru-chat e-rev",
    "hebrew": "ארוחת ערב",
    "difficulty": 5
  },
  {
    "category": "Phrases",
    "english": "Excuse me",
    "transliteration": "Sli-kha",
    "hebrew": "סליחה",
    "difficulty": 5
  },
  {
    "category": "Phrases",
    "english": "First name",
    "transliteration": "Shem pra-ti",
    "hebrew": "שם פרטי",
    "difficulty": 5
  },
  {
    "category": "Phrases",
    "english": "Get going",
    "transliteration": "Ya-la",
    "hebrew": "יאללה",
    "difficulty": 5
  },
  {
    "category": "Phrases",
    "english": "Good luck",
    "transliteration": "Be-hats-la-cha",
    "hebrew": "בהצלחה",
    "difficulty": 5
  },
  {
    "category": "Phrases",
    "english": "Hello",
    "transliteration": "Sha-lom",
    "hebrew": "שלום",
    "difficulty": 5
  },
  {
    "category": "Phrases",
    "english": "It is possible",
    "transliteration": "Ef-shar",
    "hebrew": "אפשר",
    "difficulty": 5
  },
  {
    "category": "Phrases",
    "english": "Lunch",
    "transliteration": "A-ru-chat tso-ho-ra-yim",
    "hebrew": "ארוחת צהריים",
    "difficulty": 5
  },
  {
    "category": "Phrases",
    "english": "Please",
    "transliteration": "Be-va-ka-sha",
    "hebrew": "בבקשה",
    "difficulty": 5
  },
  {
    "category": "Phrases",
    "english": "One second / hold on",
    "transliteration": "Shni-ya",
    "hebrew": "שנייה",
    "difficulty": 5
  },
  {
    "category": "Phrases",
    "english": "See you later",
    "transliteration": "Le-hit-ra-ot",
    "hebrew": "להתראות",
    "difficulty": 5
  },
  {
    "category": "Phrases",
    "english": "Thank you",
    "transliteration": "To-da",
    "hebrew": "תודה",
    "difficulty": 5
  },
  {
    "category": "Phrases",
    "english": "There is/are",
    "transliteration": "Yesh",
    "hebrew": "יש",
    "difficulty": 5
  },
  {
    "category": "Phrases",
    "english": "There isn’t/aren’t",
    "transliteration": "Ein",
    "hebrew": "אין",
    "difficulty": 5
  },
  {
    "category": "Question Words",
    "english": "How",
    "transliteration": "Eich",
    "hebrew": "איך",
    "difficulty": 5
  },
  {
    "category": "Question Words",
    "english": "How much",
    "transliteration": "Ka-ma",
    "hebrew": "כמה",
    "difficulty": 5
  },
  {
    "category": "Question Words",
    "english": "What",
    "transliteration": "Ma",
    "hebrew": "מה",
    "difficulty": 5
  },
  {
    "category": "Question Words",
    "english": "When",
    "transliteration": "Ma-tai",
    "hebrew": "מתי",
    "difficulty": 5
  },
  {
    "category": "Question Words",
    "english": "Where",
    "transliteration": "Ei-fo",
    "hebrew": "איפה",
    "difficulty": 5
  },
  {
    "category": "Question Words",
    "english": "Which",
    "transliteration": "Ei-ze",
    "hebrew": "איזה",
    "difficulty": 5
  },
  {
    "category": "Question Words",
    "english": "Why",
    "transliteration": "La-ma",
    "hebrew": "למה",
    "difficulty": 5
  }
];

const HEBREW_SUPPLEMENTAL_WORDS = [
  { category: "Numbers", english: "One", transliteration: "", hebrew: "אֶחָד", difficulty: 1 },
  { category: "Numbers", english: "Two", transliteration: "", hebrew: "שְׁנַיִם", difficulty: 1 },
  { category: "Numbers", english: "Three", transliteration: "", hebrew: "שְׁלוֹשָׁה", difficulty: 1 },
  { category: "Numbers", english: "Four", transliteration: "", hebrew: "אַרְבָּעָה", difficulty: 1 },
  { category: "Numbers", english: "Five", transliteration: "", hebrew: "חֲמִשָּׁה", difficulty: 1 },
  { category: "Numbers", english: "Six", transliteration: "", hebrew: "שִׁשָּׁה", difficulty: 1 },
  { category: "Numbers", english: "Seven", transliteration: "", hebrew: "שִׁבְעָה", difficulty: 1 },
  { category: "Numbers", english: "Eight", transliteration: "", hebrew: "שְׁמוֹנָה", difficulty: 1 },
  { category: "Numbers", english: "Nine", transliteration: "", hebrew: "תִּשְׁעָה", difficulty: 1 },
  { category: "Numbers", english: "Ten", transliteration: "", hebrew: "עֲשָׂרָה", difficulty: 1 },
  { category: "Numbers", english: "Twenty", transliteration: "", hebrew: "עֶשְׂרִים", difficulty: 2 },
  { category: "Numbers", english: "Thirty", transliteration: "", hebrew: "שְׁלוֹשִׁים", difficulty: 2 },
  { category: "Numbers", english: "Forty", transliteration: "", hebrew: "אַרְבָּעִים", difficulty: 2 },
  { category: "Numbers", english: "Fifty", transliteration: "", hebrew: "חֲמִשִּׁים", difficulty: 2 },
  { category: "Numbers", english: "Sixty", transliteration: "", hebrew: "שִׁשִּׁים", difficulty: 2 },
  { category: "Numbers", english: "Seventy", transliteration: "", hebrew: "שִׁבְעִים", difficulty: 2 },
  { category: "Numbers", english: "Eighty", transliteration: "", hebrew: "שְׁמוֹנִים", difficulty: 2 },
  { category: "Numbers", english: "Ninety", transliteration: "", hebrew: "תִּשְׁעִים", difficulty: 2 },
  { category: "Numbers", english: "Hundred", transliteration: "", hebrew: "מֵאָה", difficulty: 2 },
];

const HEBREW_WORD_REMOVALS = new Set([
  buildHebrewWordKey("אצבע", "Toe"),
]);

const HEBREW_WORD_OVERRIDES = new Map([
  [buildHebrewWordKey("פיקח", "Smart"), { english: "Clever" }],
  [buildHebrewWordKey("יד", "Arm"), { remove: true }],
  [buildHebrewWordKey("פה", "Here"), { hebrew: "פֹּה", transliteration: "" }],
  [buildHebrewWordKey("פה", "Mouth"), { hebrew: "פֶּה", transliteration: "" }],
  [buildHebrewWordKey("שם", "There"), { hebrew: "שָׁם", transliteration: "" }],
  [buildHebrewWordKey("שם", "Name"), { hebrew: "שֵׁם", transliteration: "" }],
  [buildHebrewWordKey("שמן", "Fat"), { hebrew: "שָׁמֵן", transliteration: "" }],
  [buildHebrewWordKey("שמן", "Oil"), { hebrew: "שֶׁמֶן", transliteration: "" }],
  [buildHebrewWordKey("שער", "Gate"), { hebrew: "שַׁעַר", transliteration: "" }],
  [buildHebrewWordKey("שער", "Hair"), { hebrew: "שֵׂעָר", transliteration: "" }],
]);

// Exact vocalizations for the curated beginner/intermediate bank. Keeping these
// separate from the semantic overrides above prevents the transliteration
// fallback from inventing vowel patterns.
const HEBREW_CURATED_POINTING_OVERRIDES = new Map([
  [buildHebrewWordKey("הרבה", "A lot"), "הַרְבֵּה"],
  [buildHebrewWordKey("שוב", "Again"), "שׁוּב"],
  [buildHebrewWordKey("כל", "All"), "כָּל"],
  [buildHebrewWordKey("לבד", "Alone"), "לְבַד"],
  [buildHebrewWordKey("גם", "Also"), "גַּם"],
  [buildHebrewWordKey("כועס", "Angry"), "כּוֹעֵס"],
  [buildHebrewWordKey("רע", "Bad"), "רַע"],
  [buildHebrewWordKey("משעמם", "Boring"), "מְשַׁעֲמֵם"],
  [buildHebrewWordKey("זול", "Cheap"), "זוֹל"],
  [buildHebrewWordKey("קר", "Cold"), "קַר"],
  [buildHebrewWordKey("נכון", "Correct"), "נָכוֹן"],
  [buildHebrewWordKey("למטה", "Down"), "לְמַטָּה"],
  [buildHebrewWordKey("מוקדם", "Early"), "מֻקְדָּם"],
  [buildHebrewWordKey("קל", "Easy"), "קַל"],
  [buildHebrewWordKey("מלהיב", "Exciting"), "מַלְהִיב"],
  [buildHebrewWordKey("יקר", "Expensive"), "יָקָר"],
  [buildHebrewWordKey("אסור", "Forbidden"), "אָסוּר"],
  [buildHebrewWordKey("מטוגן", "Fried"), "מְטֻגָּן"],
  [buildHebrewWordKey("מלא", "Full"), "מָלֵא"],
  [buildHebrewWordKey("מצחיק", "Funny"), "מַצְחִיק"],
  [buildHebrewWordKey("טוב", "Good"), "טוֹב"],
  [buildHebrewWordKey("חצי", "Half (of something)"), "חֵצִי"],
  [buildHebrewWordKey("בריא", "Healthy"), "בָּרִיא"],
  [buildHebrewWordKey("גבוה", "High"), "גָּבֹהַּ"],
  [buildHebrewWordKey("חם", "Hot"), "חַם"],
  [buildHebrewWordKey("רעב", "Hungry"), "רָעָב"],
  [buildHebrewWordKey("חשוב", "Important"), "חָשׁוּב"],
  [buildHebrewWordKey("נבון", "Intelligent"), "נָבוֹן"],
  [buildHebrewWordKey("יהודי", "Jewish"), "יְהוּדִי"],
  [buildHebrewWordKey("כשר", "Kosher"), "כָּשֵׁר"],
  [buildHebrewWordKey("אחרון", "Last, final"), "אַחֲרוֹן"],
  [buildHebrewWordKey("מאוחר", "Late"), "מְאֻחָר"],
  [buildHebrewWordKey("עצלן", "Lazy"), "עַצְלָן"],
  [buildHebrewWordKey("שמאלה", "Left (direction)"), "שְׂמֹאלָה"],
  [buildHebrewWordKey("נמוך", "Low"), "נָמוּךְ"],
  [buildHebrewWordKey("נשוי", "Married"), "נָשׂוּי"],
  [buildHebrewWordKey("בינוני", "Medium"), "בֵּינוֹנִי"],
  [buildHebrewWordKey("רועש", "Noisy"), "רוֹעֵשׁ"],
  [buildHebrewWordKey("בסדר", "Okay"), "בְּסֵדֶר"],
  [buildHebrewWordKey("זקן", "Old (person)"), "זָקֵן"],
  [buildHebrewWordKey("בחוץ", "Outside"), "בַּחוּץ"],
  [buildHebrewWordKey("עני", "Poor"), "עָנִי"],
  [buildHebrewWordKey("רבע", "Quarter (of something)"), "רֶבַע"],
  [buildHebrewWordKey("שקט", "Quiet"), "שָׁקֵט"],
  [buildHebrewWordKey("מוכן", "Ready"), "מוּכָן"],
  [buildHebrewWordKey("דתי", "Religious"), "דָּתִי"],
  [buildHebrewWordKey("עשיר", "Rich"), "עָשִׁיר"],
  [buildHebrewWordKey("ימינה", "Right (direction)"), "יָמִינָה"],
  [buildHebrewWordKey("חילוני", "Secular"), "חִלּוֹנִי"],
  [buildHebrewWordKey("יחיד", "Singular; single (not married)"), "יָחִיד"],
  [buildHebrewWordKey("איטי", "Slow"), "אִטִּי"],
  [buildHebrewWordKey("פיקח", "Clever"), "פִּקֵּחַ"],
  [buildHebrewWordKey("מסריח", "Smelly"), "מַסְרִיחַ"],
  [buildHebrewWordKey("רך", "Soft"), "רַךְ"],
  [buildHebrewWordKey("בקרוב", "Soon"), "בְּקָרוֹב"],
  [buildHebrewWordKey("חריף", "Spicy (hot)"), "חָרִיף"],
  [buildHebrewWordKey("ישר", "Straight ahead; honest"), "יָשָׁר"],
  [buildHebrewWordKey("מתוק", "Sweet"), "מָתוֹק"],
  [buildHebrewWordKey("טעים", "Tasty"), "טָעִים"],
  [buildHebrewWordKey("רזה", "Thin"), "רָזֶה"],
  [buildHebrewWordKey("שליש", "Third (of something)"), "שְׁלִישׁ"],
  [buildHebrewWordKey("עייף", "Tired"), "עָיֵף"],
  [buildHebrewWordKey("פעמיים", "Twice"), "פַּעֲמַיִם"],
  [buildHebrewWordKey("מכוער", "Ugly"), "מְכֹעָר"],
  [buildHebrewWordKey("למעלה", "Up"), "לְמַעְלָה"],
  [buildHebrewWordKey("מאוד", "Very"), "מְאוֹד"],
  [buildHebrewWordKey("רטוב", "Wet"), "רָטֹב"],
  [buildHebrewWordKey("שלם", "Whole"), "שָׁלֵם"],
  [buildHebrewWordKey("בלי", "Without"), "בְּלִי"],
  [buildHebrewWordKey("אתמול", "Yesterday"), "אֶתְמוֹל"],
  [buildHebrewWordKey("לפני", "Before"), "לִפְנֵי"],
  [buildHebrewWordKey("אצל", "By, with, at the place of"), "אֵצֶל"],
  [buildHebrewWordKey("אם", "If"), "אִם"],
  [buildHebrewWordKey("אולי", "Maybe"), "אוּלַי"],
  [buildHebrewWordKey("לא", "No"), "לֹא"],
  [buildHebrewWordKey("של", "Of"), "שֶׁל"],
  [buildHebrewWordKey("על", "On, about"), "עַל"],
  [buildHebrewWordKey("פעם", "Once"), "פַּעַם"],
  [buildHebrewWordKey("רק", "Only"), "רַק"],
  [buildHebrewWordKey("או", "Or"), "אוֹ"],
  [buildHebrewWordKey("אז", "So"), "אָז"],
  [buildHebrewWordKey("זה", "This"), "זֶה"],
  [buildHebrewWordKey("כתובת", "Address"), "כְּתֹבֶת"],
  [buildHebrewWordKey("מזגן", "Air conditioner"), "מַזְגָן"],
  [buildHebrewWordKey("נמל תעופה", "Airport"), "נְמַל תְּעוּפָה"],
  [buildHebrewWordKey("שקד", "Almond"), "שָׁקֵד"],
  [buildHebrewWordKey("תשובה", "Answer"), "תְּשׁוּבָה"],
  [buildHebrewWordKey("דירה", "Apartment"), "דִּירָה"],
  [buildHebrewWordKey("ערבית", "Arabic"), "עַרְבִית"],
  [buildHebrewWordKey("כספומט", "ATM"), "כַּסְפּוֹמָט"],
  [buildHebrewWordKey("דודה", "Aunt"), "דּוֹדָה"],
  [buildHebrewWordKey("גב", "Back"), "גַּב"],
  [buildHebrewWordKey("מאפייה", "Bakery"), "מַאֲפִיָּה"],
  [buildHebrewWordKey("בנק", "Bank"), "בַּנְק"],
  [buildHebrewWordKey("כדורסל", "Basketball"), "כַּדּוּרְסַל"],
  [buildHebrewWordKey("חוף", "Beach"), "חוֹף"],
  [buildHebrewWordKey("בקר", "Beef"), "בָּקָר"],
  [buildHebrewWordKey("בירה", "Beer"), "בִּירָה"],
  [buildHebrewWordKey("סלק", "Beetroot"), "סֶלֶק"],
  [buildHebrewWordKey("חגורה", "Belt"), "חֲגוֹרָה"],
  [buildHebrewWordKey("חשבון", "Bill, check"), "חֶשְׁבּוֹן"],
  [buildHebrewWordKey("גוף", "Body"), "גּוּף"],
  [buildHebrewWordKey("עצם", "Bone"), "עֶצֶם"],
  [buildHebrewWordKey("בקבוק", "Bottle"), "בַּקְבּוּק"],
  [buildHebrewWordKey("מוח", "Brain"), "מוֹחַ"],
  [buildHebrewWordKey("הפסקה", "Break"), "הַפְסָקָה"],
  [buildHebrewWordKey("חום", "Brown"), "חוּם"],
  [buildHebrewWordKey("בניין", "Building"), "בִּנְיָן"],
  [buildHebrewWordKey("חמאה", "Butter"), "חֶמְאָה"],
  [buildHebrewWordKey("כרוב", "Cabbage"), "כְּרוּב"],
  [buildHebrewWordKey("תקרה", "Ceiling"), "תִּקְרָה"],
  [buildHebrewWordKey("חלה", "Challah"), "חַלָּה"],
  [buildHebrewWordKey("ערוץ", "Channel (tv)"), "עָרוּץ"],
  [buildHebrewWordKey("פרק", "Chapter"), "פֶּרֶק"],
  [buildHebrewWordKey("חזה", "Chest"), "חָזֶה"],
  [buildHebrewWordKey("ג׳וק", "Cockroach"), "ג׳וּק"],
  [buildHebrewWordKey("צבע", "Color"), "צֶבַע"],
  [buildHebrewWordKey("חברה", "Company"), "חֶבְרָה"],
  [buildHebrewWordKey("מדינה", "Country"), "מְדִינָה"],
  [buildHebrewWordKey("בן דוד", "Cousin"), "בֶּן דּוֹד"],
  [buildHebrewWordKey("מלפפון", "Cucumber"), "מְלָפְפוֹן"],
  [buildHebrewWordKey("תמר", "Date (the fruit)"), "תָּמָר"],
  [buildHebrewWordKey("יום", "Day"), "יוֹם"],
  [buildHebrewWordKey("מדבר", "Desert"), "מִדְבָּר"],
  [buildHebrewWordKey("מילון", "Dictionary"), "מִלּוֹן"],
  [buildHebrewWordKey("רופא", "Doctor / physician"), "רוֹפֵא"],
  [buildHebrewWordKey("שתייה", "Drink"), "שְׁתִיָּה"],
  [buildHebrewWordKey("נהג", "Driver"), "נַהָג"],
  [buildHebrewWordKey("מזרח", "East"), "מִזְרָח"],
  [buildHebrewWordKey("סוף", "End"), "סוֹף"],
  [buildHebrewWordKey("מהנדס", "Engineer"), "מְהַנְדֵּס"],
  [buildHebrewWordKey("אנגלית", "English"), "אַנְגְּלִית"],
  [buildHebrewWordKey("כניסה", "Entrance"), "כְּנִיסָה"],
  [buildHebrewWordKey("ערב", "Evening"), "עֶרֶב"],
  [buildHebrewWordKey("מצוין", "Excellent"), "מְצֻיָּן"],
  [buildHebrewWordKey("התעמלות", "Exercise"), "הִתְעַמְּלוּת"],
  [buildHebrewWordKey("יציאה", "Exit"), "יְצִיאָה"],
  [buildHebrewWordKey("פקולטה", "Faculty"), "פָקוּלְטָה"],
  [buildHebrewWordKey("פלאפל", "Falafel"), "פָלָאפֶל"],
  [buildHebrewWordKey("סתיו", "Fall (the season)"), "סְתָו"],
  [buildHebrewWordKey("תאנה", "Fig"), "תְּאֵנָה"],
  [buildHebrewWordKey("אצבע", "Finger"), "אֶצְבַּע"],
  [buildHebrewWordKey("רצפה", "Floor"), "רִצְפָּה"],
  [buildHebrewWordKey("כף רגל", "Foot"), "כַּף רֶגֶל"],
  [buildHebrewWordKey("יום שישי", "Friday"), "יוֹם שִׁשִּׁי"],
  [buildHebrewWordKey("פרי", "Fruit"), "פְּרִי"],
  [buildHebrewWordKey("עתיד", "Future"), "עָתִיד"],
  [buildHebrewWordKey("משחק", "Game"), "מִשְׂחָק"],
  [buildHebrewWordKey("פח אשפה", "Garbage can"), "פַּח אַשְׁפָּה"],
  [buildHebrewWordKey("שום", "Garlic"), "שׁוּם"],
  [buildHebrewWordKey("דלק", "Gasoline"), "דֶּלֶק"],
  [buildHebrewWordKey("ממשלה", "Government"), "מֶמְשָׁלָה"],
  [buildHebrewWordKey("סבא", "Grandfather"), "סַבָּא"],
  [buildHebrewWordKey("סבתא", "Grandmother"), "סָבְתָא"],
  [buildHebrewWordKey("אשכולית", "Grapefruit"), "אֶשְׁכּוֹלִית"],
  [buildHebrewWordKey("קבוצה", "Group"), "קְבוּצָה"],
  [buildHebrewWordKey("יד", "Hand"), "יָד"],
  [buildHebrewWordKey("לב", "Heart"), "לֵב"],
  [buildHebrewWordKey("עברית", "Hebrew"), "עִבְרִית"],
  [buildHebrewWordKey("גובה", "Height"), "גֹּבַהּ"],
  [buildHebrewWordKey("תחביב", "Hobby"), "תַּחְבִּיב"],
  [buildHebrewWordKey("חג", "Holiday"), "חַג"],
  [buildHebrewWordKey("בית חולים", "Hospital"), "בֵּית חוֹלִים"],
  [buildHebrewWordKey("שוקו חם", "Hot chocolate"), "שׁוֹקוֹ חַם"],
  [buildHebrewWordKey("מלון", "Hotel"), "מָלוֹן"],
  [buildHebrewWordKey("חומוס", "Hummus"), "חוּמוּס"],
  [buildHebrewWordKey("בעל", "Husband"), "בַּעַל"],
  [buildHebrewWordKey("צה״ל", "IDF (Israel Defense Forces)"), "צָהָ״ל"],
  [buildHebrewWordKey("רעיון", "Idea"), "רַעֲיוֹן"],
  [buildHebrewWordKey("סכין", "Knife"), "סַכִּין"],
  [buildHebrewWordKey("טלה", "Lamb"), "טָלֶה"],
  [buildHebrewWordKey("שפה", "Language"), "שָׂפָה"],
  [buildHebrewWordKey("חסה", "Lettuce"), "חַסָּה"],
  [buildHebrewWordKey("לטאה", "Lizard"), "לְטָאָה"],
  [buildHebrewWordKey("דואר", "Mail; post office"), "דֹּאַר"],
  [buildHebrewWordKey("קניון", "Mall"), "קַנְיוֹן"],
  [buildHebrewWordKey("מנהל", "Manager"), "מְנַהֵל"],
  [buildHebrewWordKey("מנגו", "Mango"), "מַנְגּוֹ"],
  [buildHebrewWordKey("בשר", "Meat"), "בָּשָׂר"],
  [buildHebrewWordKey("מטר", "Meter"), "מֶטֶר"],
  [buildHebrewWordKey("מיקרוגל", "Microwave"), "מִיקְרוֹגַל"],
  [buildHebrewWordKey("דקה", "Minute"), "דַּקָּה"],
  [buildHebrewWordKey("עובש", "Mold"), "עֹבֶשׁ"],
  [buildHebrewWordKey("רגע", "Moment"), "רֶגַע"],
  [buildHebrewWordKey("יום שני", "Monday"), "יוֹם שֵׁנִי"],
  [buildHebrewWordKey("חודש", "Month"), "חֹדֶשׁ"],
  [buildHebrewWordKey("סרט", "Movie"), "סֶרֶט"],
  [buildHebrewWordKey("מוזיאון", "Museum"), "מוּזֵאוֹן"],
  [buildHebrewWordKey("פטריות", "Mushrooms"), "פִּטְרִיּוֹת"],
  [buildHebrewWordKey("מוזיקה", "Music"), "מוּזִיקָה"],
  [buildHebrewWordKey("מפית", "Napkin"), "מַפִּית"],
  [buildHebrewWordKey("נקטרינה", "Nectarine"), "נֶקְטָרִינָה"],
  [buildHebrewWordKey("שכונה", "Neighborhood"), "שְׁכוּנָה"],
  [buildHebrewWordKey("חדשות", "News"), "חֲדָשׁוֹת"],
  [buildHebrewWordKey("עיתון", "Newspaper"), "עִתּוֹן"],
  [buildHebrewWordKey("צהריים", "Noon"), "צָהֳרַיִם"],
  [buildHebrewWordKey("צפון", "North"), "צָפוֹן"],
  [buildHebrewWordKey("אף", "Nose"), "אַף"],
  [buildHebrewWordKey("מספר", "Number"), "מִסְפָּר"],
  [buildHebrewWordKey("משרד", "Office"), "מִשְׂרָד"],
  [buildHebrewWordKey("זית", "Olive"), "זַיִת"],
  [buildHebrewWordKey("בצל", "Onion"), "בָּצָל"],
  [buildHebrewWordKey("תנור", "Oven"), "תַּנּוּר"],
  [buildHebrewWordKey("כאב", "Pain/ache"), "כְּאֵב"],
  [buildHebrewWordKey("כף יד", "Palm"), "כַּף יָד"],
  [buildHebrewWordKey("מכנסיים", "Pants"), "מִכְנָסַיִם"],
  [buildHebrewWordKey("הורים", "Parents"), "הוֹרִים"],
  [buildHebrewWordKey("פטרוזיליה", "Parsley"), "פֶּטְרוֹזִילְיָה"],
  [buildHebrewWordKey("דרכון", "Passport"), "דַּרְכּוֹן"],
  [buildHebrewWordKey("עבר", "Past"), "עָבָר"],
  [buildHebrewWordKey("מאפה", "Pastry"), "מַאֲפֶה"],
  [buildHebrewWordKey("פלפל", "Pepper"), "פִּלְפֵּל"],
  [buildHebrewWordKey("בית מרקחת", "Pharmacy"), "בֵּית מִרְקַחַת"],
  [buildHebrewWordKey("פסנתר", "Piano"), "פְּסַנְתֵּר"],
  [buildHebrewWordKey("חמוצים", "Pickles"), "חֲמוּצִים"],
  [buildHebrewWordKey("כרית", "Pillow"), "כָּרִית"],
  [buildHebrewWordKey("פיתה", "Pita"), "פִּתָּה"],
  [buildHebrewWordKey("מקום", "Place"), "מָקוֹם"],
  [buildHebrewWordKey("צלחת", "Plate"), "צַלַּחַת"],
  [buildHebrewWordKey("שזיף", "Plum"), "שְׁזִיף"],
  [buildHebrewWordKey("משטרה", "Police"), "מִשְׁטָרָה"],
  [buildHebrewWordKey("רימון", "Pomegranate"), "רִמּוֹן"],
  [buildHebrewWordKey("הווה", "Present (tense)"), "הוֹוֶה"],
  [buildHebrewWordKey("שאלה", "Question"), "שְׁאֵלָה"],
  [buildHebrewWordKey("קינואה", "Quinoa"), "קִינוֹאָה"],
  [buildHebrewWordKey("רב", "Rabbi"), "רַב"],
  [buildHebrewWordKey("גשם", "Rain"), "גֶּשֶׁם"],
  [buildHebrewWordKey("קבלה", "Receipt"), "קַבָּלָה"],
  [buildHebrewWordKey("מקרר", "Refrigerator"), "מְקָרֵר"],
  [buildHebrewWordKey("מסעדה", "Restaurant"), "מִסְעָדָה"],
  [buildHebrewWordKey("נהר", "River"), "נָהָר"],
  [buildHebrewWordKey("כביש", "Road, route"), "כְּבִישׁ"],
  [buildHebrewWordKey("חדר", "Room"), "חֶדֶר"],
  [buildHebrewWordKey("שטיח", "Rug"), "שָׁטִיחַ"],
  [buildHebrewWordKey("סלט", "Salad"), "סָלָט"],
  [buildHebrewWordKey("מלח", "Salt"), "מֶלַח"],
  [buildHebrewWordKey("סנדלים", "Sandals"), "סַנְדָּלִים"],
  [buildHebrewWordKey("שבת", "Saturday"), "שַׁבָּת"],
  [buildHebrewWordKey("רוטב", "Sauce"), "רֹטֶב"],
  [buildHebrewWordKey("שניצל", "Schnitzel"), "שְׁנִיצֶל"],
  [buildHebrewWordKey("מדען", "Scientist"), "מַדְּעָן"],
  [buildHebrewWordKey("מנה", "Serving/portion"), "מָנָה"],
  [buildHebrewWordKey("שקשוקה", "Shakshuka"), "שַׁקְשׁוּקָה"],
  [buildHebrewWordKey("שווארמה", "Shawarma"), "שָׁוַארְמָה"],
  [buildHebrewWordKey("סדין", "Sheet"), "סָדִין"],
  [buildHebrewWordKey("שקל", "Shekel"), "שֶׁקֶל"],
  [buildHebrewWordKey("מכנסיים קצרים", "Shorts"), "מִכְנָסַיִם קְצָרִים"],
  [buildHebrewWordKey("שירות", "Shuttle taxi"), "שֵׁרוּת"],
  [buildHebrewWordKey("מדרכה", "Sidewalk"), "מִדְרָכָה"],
  [buildHebrewWordKey("חצאית", "Skirt"), "חֲצָאִית"],
  [buildHebrewWordKey("שמיים", "Sky"), "שָׁמַיִם"],
  [buildHebrewWordKey("כדורגל", "Soccer"), "כַּדּוּרֶגֶל"],
  [buildHebrewWordKey("גרביים", "Socks"), "גַּרְבַּיִם"],
  [buildHebrewWordKey("מישהו", "Someone"), "מִישֶׁהוּ"],
  [buildHebrewWordKey("משהו", "Something"), "מַשֶּׁהוּ"],
  [buildHebrewWordKey("דרום", "South"), "דָּרוֹם"],
  [buildHebrewWordKey("תרד", "Spinach"), "תֶּרֶד"],
  [buildHebrewWordKey("ספורט", "Sport"), "סְפּוֹרְט"],
  [buildHebrewWordKey("אביב", "Spring"), "אָבִיב"],
  [buildHebrewWordKey("קישוא", "Squash, zucchini"), "קִשּׁוּא"],
  [buildHebrewWordKey("תחנה", "Station, stop"), "תַּחֲנָה"],
  [buildHebrewWordKey("בטן", "Stomach"), "בֶּטֶן"],
  [buildHebrewWordKey("עצור", "Stop"), "עֲצֹר"],
  [buildHebrewWordKey("חנות", "Store"), "חֲנוּת"],
  [buildHebrewWordKey("רחוב", "Street"), "רְחוֹב"],
  [buildHebrewWordKey("סטודנט", "Student"), "סְטוּדֵנְט"],
  [buildHebrewWordKey("סוכר", "Sugar"), "סֻכָּר"],
  [buildHebrewWordKey("קיץ", "Summer"), "קַיִץ"],
  [buildHebrewWordKey("יום ראשון", "Sunday"), "יוֹם רִאשׁוֹן"],
  [buildHebrewWordKey("סופרמרקט", "Supermarket"), "סוּפֶּרְמַרְקֶט"],
  [buildHebrewWordKey("שם משפחה", "Surname"), "שֵׁם מִשְׁפָּחָה"],
  [buildHebrewWordKey("בטטה", "Sweet potato"), "בָּטָטָה"],
  [buildHebrewWordKey("שחייה", "Swimming"), "שְׂחִיָּה"],
  [buildHebrewWordKey("בית כנסת", "Synagogue"), "בֵּית כְּנֶסֶת"],
  [buildHebrewWordKey("מונית", "Taxi"), "מוֹנִית"],
  [buildHebrewWordKey("מורה", "Teacher"), "מוֹרֶה"],
  [buildHebrewWordKey("מבחן", "Test/exam"), "מִבְחָן"],
  [buildHebrewWordKey("אגודל", "Thumb"), "אֲגוּדָל"],
  [buildHebrewWordKey("יום חמישי", "Thursday"), "יוֹם חֲמִישִׁי"],
  [buildHebrewWordKey("שירותים", "Bathroom / toilets"), "שֵׁרוּתִים"],
  [buildHebrewWordKey("מגבת", "Towel"), "מַגֶּבֶת"],
  [buildHebrewWordKey("תנועה", "Traffic"), "תְּנוּעָה"],
  [buildHebrewWordKey("טיול", "Trip, hike"), "טִיּוּל"],
  [buildHebrewWordKey("יום שלישי", "Tuesday"), "יוֹם שְׁלִישִׁי"],
  [buildHebrewWordKey("מנהרה", "Tunnel"), "מִנְהָרָה"],
  [buildHebrewWordKey("הודו", "India"), "הֹדּוּ"],
  [buildHebrewWordKey("דוד", "Uncle"), "דּוֹד"],
  [buildHebrewWordKey("אוניברסיטה", "University"), "אוּנִיבֶרְסִיטָה"],
  [buildHebrewWordKey("חופשה", "Vacation"), "חֻפְשָׁה"],
  [buildHebrewWordKey("ירקות", "Vegetables"), "יְרָקוֹת"],
  [buildHebrewWordKey("קיר", "Wall"), "קִיר"],
  [buildHebrewWordKey("ארנק", "Wallet"), "אַרְנָק"],
  [buildHebrewWordKey("אגוז מלך", "Walnut"), "אֱגוֹז מֶלֶךְ"],
  [buildHebrewWordKey("נשק", "Weapon"), "נֶשֶׁק"],
  [buildHebrewWordKey("יום רביעי", "Wednesday"), "יוֹם רְבִיעִי"],
  [buildHebrewWordKey("סוף שבוע", "Weekend"), "סוֹף שָׁבוּעַ"],
  [buildHebrewWordKey("משקל", "Weight"), "מִשְׁקָל"],
  [buildHebrewWordKey("מערב", "West"), "מַעֲרָב"],
  [buildHebrewWordKey("חלון", "Window"), "חַלּוֹן"],
  [buildHebrewWordKey("יין", "Wine"), "יַיִן"],
  [buildHebrewWordKey("חורף", "Winter"), "חֹרֶף"],
  [buildHebrewWordKey("עולם", "World"), "עוֹלָם"],
  [buildHebrewWordKey("שנה", "Year"), "שָׁנָה"],
  [buildHebrewWordKey("יוגורט", "Yogurt"), "יוֹגוּרְט"],
  [buildHebrewWordKey("לבריאות", "Bless you"), "לַבְּרִיאוּת"],
  [buildHebrewWordKey("לחיים", "Cheers!"), "לַחַיִּים"],
  [buildHebrewWordKey("תאריך לידה", "Date of birth"), "תַּאֲרִיךְ לֵדָה"],
  [buildHebrewWordKey("שם פרטי", "First name"), "שֵׁם פְּרָטִי"],
  [buildHebrewWordKey("יאללה", "Get going"), "יַאלְלָה"],
  [buildHebrewWordKey("בהצלחה", "Good luck"), "בְּהַצְלָחָה"],
  [buildHebrewWordKey("אפשר", "It is possible"), "אֶפְשָׁר"],
  [buildHebrewWordKey("ארוחת צהריים", "Lunch"), "אֲרוּחַת צָהֳרַיִם"],
  [buildHebrewWordKey("שנייה", "One second / hold on"), "שְׁנִיָּה"],
  [buildHebrewWordKey("להתראות", "See you later"), "לְהִתְרָאוֹת"],
  [buildHebrewWordKey("אין", "There isn’t/aren’t"), "אֵין"],
  [buildHebrewWordKey("איזה", "Which"), "אֵיזֶה"],
]);

const HEBREW_LEVEL_1_KEYS = new Set(
  [
    ["טוב", "Good"],
    ["רע", "Bad"],
    ["גדול", "Big"],
    ["קטן", "Small"],
    ["חדש", "New"],
    ["ישן", "Old (thing)"],
    ["יפה", "Beautiful"],
    ["חם", "Hot"],
    ["קר", "Cold"],
    ["נקי", "Clean"],
    ["פתוח", "Open"],
    ["סגור", "Closed"],
    ["אדום", "Red"],
    ["כחול", "Blue"],
    ["ירוק", "Green"],
    ["צהוב", "Yellow"],
    ["שחור", "Black"],
    ["לבן", "White"],
    ["כתום", "Orange (color)"],
    ["שלום", "Hello"],
    ["תודה", "Thank you"],
    ["בבקשה", "Please"],
    ["סליחה", "Excuse me"],
    ["כן", "Yes"],
    ["לא", "No"],
    ["זה", "This"],
    ["מה", "What"],
    ["מי", "Who"],
    ["איפה", "Where"],
    ["יש", "There is/are"],
    ["אין", "There isn’t/aren’t"],
    ["אני", "I"],
    ["הוא", "He"],
    ["אבא", "Dad"],
    ["אימא", "Mom"],
    ["ילד", "Boy"],
    ["ילדה", "Girl"],
    ["בית", "House"],
    ["מים", "Water"],
    ["לחם", "Bread"],
    ["חלב", "Milk"],
    ["תפוח", "Apple"],
    ["בננה", "Banana"],
    ["חתול", "Cat"],
    ["כלב", "Dog"],
    ["ספר", "Book"],
    ["תיק", "Bag"],
    ["כיסא", "Chair"],
    ["שולחן", "Table"],
    ["דלת", "Door"],
    ["חלון", "Window"],
    ["מיטה", "Bed"],
    ["כוס", "Glass, cup"],
    ["צלחת", "Plate"],
    ["היום", "Today"],
    ["מחר", "Tomorrow"],
    ["כל", "All"],
    ["גם", "Also"],
    ["אֶחָד", "One"],
    ["שְׁנַיִם", "Two"],
    ["שְׁלוֹשָׁה", "Three"],
    ["אַרְבָּעָה", "Four"],
    ["חֲמִישָּׁה", "Five"],
    ["שִׁשָּׁה", "Six"],
    ["שִׁבְעָה", "Seven"],
    ["שְׁמוֹנָה", "Eight"],
    ["תִּשְׁעָה", "Nine"],
    ["עֲשָׂרָה", "Ten"],
  ].map(([hebrew, english]) => buildHebrewWordKey(hebrew, english))
);

const HEBREW_LEVEL_2_KEYS = new Set(
  [
    ["איך", "How"],
    ["כמה", "How much"],
    ["מתי", "When"],
    ["למה", "Why"],
    ["איזה", "Which"],
    ["עכשיו", "Now"],
    ["אתמול", "Yesterday"],
    ["לפני", "Before"],
    ["שוב", "Again"],
    ["ביחד", "Together"],
    ["או", "Or"],
    ["אבל", "But"],
    ["של", "Of"],
    ["על", "On, about"],
    ["ראשון", "First"],
    ["הבא", "Next"],
    ["מהר", "Quickly"],
    ["לאט", "Slowly"],
    ["מהיר", "Fast"],
    ["פשוט", "Simple"],
    ["חזק", "Strong"],
    ["חלש", "Weak"],
    ["סגול", "Purple"],
    ["חום", "Brown"],
    ["מטבח", "Kitchen"],
    ["חדר", "Room"],
    ["בית ספר", "School"],
    ["בקבוק", "Bottle"],
    ["כדור", "Ball"],
    ["שעון", "Clock"],
    ["עט", "Pen"],
    ["עיפרון", "Pencil"],
    ["כף", "Spoon"],
    ["מזלג", "Fork"],
    ["סכין", "Knife"],
    ["כיתה", "Classroom"],
    ["מחשב", "Computer"],
    ["גבינה", "Cheese"],
    ["עוגה", "Cake"],
    ["קפה", "Coffee"],
    ["שוקולד", "Chocolate"],
    ["ביצה", "Egg"],
    ["דג", "Fish"],
    ["פרי", "Fruit"],
    ["משפחה", "Family"],
    ["יום", "Day"],
    ["שעה", "Hour"],
    ["חבר", "Friend"],
    ["ראש", "Head"],
    ["יד", "Hand"],
    ["עין", "Eye"],
    ["אוזן", "Ear"],
    ["פֶּה", "Mouth"],
    ["פֹּה", "Here"],
    ["שֵׁם", "Name"],
    ["שָׁם", "There"],
    ["עֶשְׂרִים", "Twenty"],
    ["שְׁלוֹשִׁים", "Thirty"],
    ["אַרְבָּעִים", "Forty"],
    ["חֲמִשִּׁים", "Fifty"],
    ["שִׁשִּׁים", "Sixty"],
    ["שִׁבְעִים", "Seventy"],
    ["שְׁמוֹנִים", "Eighty"],
    ["תִּשְׁעִים", "Ninety"],
    ["מֵאָה", "Hundred"],
  ].map(([hebrew, english]) => buildHebrewWordKey(hebrew, english))
);

const RAW_HEBREW_EXPANDED_WORDS = Array.isArray(
  typeof HEBREW_EXPANDED_WORDS !== "undefined" ? HEBREW_EXPANDED_WORDS : null
)
  ? HEBREW_EXPANDED_WORDS
  : [];

const HEBREW_WORDS = mergeHebrewWordSets(
  buildCuratedHebrewWords(RAW_HEBREW_WORDS),
  normalizeExpandedHebrewWords(RAW_HEBREW_EXPANDED_WORDS)
);

function buildHebrewWordKey(hebrew, english) {
  return `${String(hebrew || "").trim()}||${String(english || "").trim()}`;
}

function applyHebrewWordOverride(entry) {
  const override = HEBREW_WORD_OVERRIDES.get(buildHebrewWordKey(entry.hebrew, entry.english));
  const semanticallyCorrectedEntry = override ? { ...entry, ...override } : entry;
  const pointedHebrew = HEBREW_CURATED_POINTING_OVERRIDES.get(
    buildHebrewWordKey(semanticallyCorrectedEntry.hebrew, semanticallyCorrectedEntry.english)
  );
  return pointedHebrew
    ? { ...semanticallyCorrectedEntry, hebrew: pointedHebrew, transliteration: "" }
    : semanticallyCorrectedEntry;
}

function getCuratedHebrewDifficulty(entry, rawEntry) {
  const keys = [
    buildHebrewWordKey(entry.hebrew, entry.english),
    buildHebrewWordKey(rawEntry?.hebrew, rawEntry?.english),
  ];
  if (keys.some((key) => HEBREW_LEVEL_1_KEYS.has(key))) {
    return 1;
  }
  if (keys.some((key) => HEBREW_LEVEL_2_KEYS.has(key))) {
    return 2;
  }

  const difficulty = Number(rawEntry?.difficulty);
  if (!Number.isFinite(difficulty)) {
    return 3;
  }

  return difficulty <= 2 ? 3 : Math.max(3, Math.min(5, difficulty));
}

function buildCuratedHebrewWords(words) {
  const seen = new Set();
  const curatedWords = [];

  [...words, ...HEBREW_SUPPLEMENTAL_WORDS].forEach((rawEntry) => {
    if (HEBREW_WORD_REMOVALS.has(buildHebrewWordKey(rawEntry.hebrew, rawEntry.english))) {
      return;
    }

    const entry = applyHebrewWordOverride(rawEntry);
    if (entry.remove) {
      return;
    }

    const curatedEntry = {
      category: entry.category,
      english: String(entry.english || "").trim(),
      transliteration: String(entry.transliteration || "").trim(),
      hebrew: String(entry.hebrew || "").trim(),
      difficulty: getCuratedHebrewDifficulty(entry, rawEntry),
    };

    const identity = buildHebrewWordKey(curatedEntry.hebrew, curatedEntry.english);
    if (!curatedEntry.hebrew || !curatedEntry.english || seen.has(identity)) {
      return;
    }

    seen.add(identity);
    curatedWords.push(curatedEntry);
  });

  return curatedWords;
}

function normalizeExpandedHebrewWords(words) {
  const normalizedWords = [];

  (words || []).forEach((entry) => {
    const difficulty = Math.max(6, Math.min(10, Number(entry?.difficulty) || 6));
    const normalizedEntry = {
      category: String(entry?.category || "Everyday Hebrew (Expanded)").trim(),
      english: String(entry?.english || "").trim(),
      transliteration: String(entry?.transliteration || "").trim(),
      hebrew: String(entry?.hebrew || "").trim(),
      difficulty,
    };

    if (!normalizedEntry.hebrew || !normalizedEntry.english) {
      return;
    }

    normalizedWords.push(normalizedEntry);
  });

  return normalizedWords;
}

function mergeHebrewWordSets(...wordSets) {
  const mergedWords = [];
  const seen = new Set();

  wordSets.flat().forEach((entry) => {
    const identity = buildHebrewWordKey(entry?.hebrew, entry?.english);
    if (!entry?.hebrew || !entry?.english || seen.has(identity)) {
      return;
    }

    seen.add(identity);
    mergedWords.push(entry);
  });

  return mergedWords;
}
