(() => {
  ...
  function renderPaintSwirlMatchingQuestion(
    question,
    ...
    function buildCurvePath(start, end, curveSeed = 0) {
      const horizontalDistance = Math.max(1, Math.abs(end.x - start.x));
      const verticalDifference = end.y - start.y;
      const bend = Math.max(-24, Math.min(24, verticalDifference * 0.14)) +
        (curveSeed % 2 === 0 ? -4 : 4);
      const firstControlX = start.x + horizontalDistance * 0.38;
      const secondControlX = end.x - horizontalDistance * 0.38;
      return [
        `M ${start.x.toFixed(1)} ${start.y.toFixed(1)}`,
        `C ${firstControlX.toFixed(1)} ${(start.y + bend).toFixed(1)},`,
        `${secondControlX.toFixed(1)} ${(end.y - bend).toFixed(1)},`,
    ...
    function appendPaintLine(start, end, leftIndex, { preview = false, snapped = false } = {}) {
      const d = buildCurvePath(start, end, leftIndex);
      const previewClasses = preview ? ` preview${snapped ? " snapped" : ""}` : "";
      const color = preview ? null : pairColor(leftIndex);
      appendPath(d, `matching-line paint-under${previewClasses}`, color);
      appendPath(d, `matching-line paint-main${previewClasses}`, color);
      if (!preview) {
        appendPath(d, "matching-line paint-texture", color);
      }
    }
    ...
    function applyRowColor(row, anchor, leftIndex) {
      ...
      if (color) {
        anchor.dataset.paintColor = color.color;
      } else {
        delete anchor.dataset.paintColor;
      }
    }
    ...
    function renderLines() {
      ...
      if (dragSession?.point) {
        ...
        const colorIndex = dragSession.source.side === "left"
          ...
          : Math.max(0, getConnectionForRight(dragSession.source.index));
        appendPaintLine(start, end, colorIndex, {
          preview: true,
  ...
  window.renderMatchingDragQuestion = renderPaintSwirlMatchingQuestion;
})();
